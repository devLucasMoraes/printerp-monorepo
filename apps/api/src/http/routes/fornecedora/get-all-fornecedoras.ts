...
export async function getAllFornecedoras(app: FastifyInstance) {
  app
    ...
    .register(auth)
    .get(
      '/api/v1/organizations/:orgSlug/fornecedoras',
      {
        schema: {
          tags: ['fornecedoras'],
          summary: 'Get all fornecedoras',
          security: [{ bearerAuth: [] }],
          params: z.object({
            orgSlug: z.string(),
          }),
      ...
      async (req, res) => {
        ...
        const fornecedoras = await getAllFornecedorasUseCase.execute(membership)

        return res.status(201).send(fornecedoras)
      },
    )
}
